...
export function buildMonthRows(
  ...
  year: number,
  month: number,
  allLogsByPerson: Map<string, ActivityLog[]>,
): PersonMonthRow[] {
  ...
  return people.map((person) => {
    const personLogs = allLogsByPerson.get(person.id) ?? [];
    return {
      person,
      currentStreak: currentStreak(personLogs),
      days: days.map((date) => {
        ...
        return {
          ...
          activeRecovery,
          hasActivity:
            weightTraining || cardio || sport || activeRecovery,
        };
      }),
    };
  });
